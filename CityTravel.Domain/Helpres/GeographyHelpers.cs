...
namespace CityTravel.Domain.Helpres
{
    ...
    public static class GeographyHelpers
    {
        ...
        {
            return geography ?? SqlGeography.STGeomFromWKB(new SqlBytes(binGeography), 4326);
        }
        ...
        public static byte[] SetBinFromGeography(SqlGeography value, string type)
        {
            ...
            switch (type)
            {
                case "stop":
                    ...
                    ? SqlGeography.STPolyFromText(value.STAsText(), 4326).STAsBinary().Buffer
                    : SqlGeography.STPointFromText(value.STAsText(), 4326).STAsBinary().Buffer;
                    break;
                case "route":
                    returnBinary = SqlGeography.STLineFromText(value.STAsText(), 4326).STAsBinary().Buffer;
                    break;
            }
