...
using CityTravel.Domain.Repository;
    ...
    public class AddRouteController : BaseController
    {
        ...
        /// The route provider. 
        /// </param>
        public AddRouteController(IUnitOfWork unitOfWork)
        {
            this.routeRepository = unitOfWork.RouteRepository;
            this.stopRepository = unitOfWork.StopRepository;
        }
            ...
            foreach (var route in routes)
            {
                ...
                for (var i = 1; i <= route.RouteGeography.STNumPoints(); i++)
                {
                    coords.Add(
                        new
                            {
                                lat = route.RouteGeography.STPointN(i).GetLatitude(),
                                lng = route.RouteGeography.STPointN(i).GetLongitude(),
                            });
                }
            ...
            foreach (var stop in stops)
            {
                stopsList.Add(
                    new
                        {
                            name = stop.Name,
                            lat = stop.StopGeography.EnvelopeCenter().GetLatitude(),
                            lng = stop.StopGeography.EnvelopeCenter().GetLongitude()
                        });
            }
